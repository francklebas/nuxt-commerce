...
const checkout = async () => {
  ...
  try {
    const response = await $fetch<{ url: string }>('/api/checkout-session', {
      method: 'POST',
      body: {
        items: cartStore.items.map((item) => ({ id: item.id, size: item.size, quantity: item.quantity }))
      }
    })
...
<template>
  <main class="mx-auto max-w-5xl px-4 py-12 sm:px-6">
    ...
    <div v-if="!cartStore.items.length" class="rounded-3xl bg-white/80 p-10 text-center">
      ...
      <NuxtLink :to="localePath('/boutique')" class="mt-5 inline-flex rounded-full bg-clay-700 px-6 py-3 text-sm font-bold uppercase tracking-[0.15em] text-white">
        {{ t('cart.backToShop') }}
      </NuxtLink>
    </div>
    ...
    <section v-else class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
      <div class="space-y-4">
        ...
        >
          ...
          <div class="flex-1">
            <NuxtLink :to="localePath(`/produit/${item.slug}`)" class="text-xl text-ink-900 hover:text-clay-700">{{ item.name }}</NuxtLink>
